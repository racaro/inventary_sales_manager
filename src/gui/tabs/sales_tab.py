        ...
        try:
            ...
            for row_idx, row_data in self.data.iterrows():
                ...
                for col_idx, col_name in enumerate(self.handler.columns):
                    ...
                    # Add dropdowns in specific columns - PRODUCTS UPDATED
                    if col_name == "Producto":
                        combo = QComboBox()
                        combo.addItems(["Sembra 2023", "Otro"])  # CHANGED
    ...
    def on_table_edit(self, item):
        ...
        try:
            ...
            if column_name in ["Cantidad vendida", "Precio total venta", "Precio unitario"]:
                if new_value.strip() == "":
                    new_value = 0 if column_name == "Cantidad vendida" else 0.0
                else:
                    try:
                        new_value = int(new_value) if column_name == "Cantidad vendida" else float(new_value)
                    except ValueError:
                        new_value = 0 if column_name == "Cantidad vendida" else 0.0

                # Ensure the column has the right dtype before assignment
                if column_name == "Cantidad vendida":
                    self.data["Cantidad vendida"] = self.data["Cantidad vendida"].astype('int64')
                else:
                ...
                if column_name == "Cantidad vendida":
                    ...
                    if not pd.isna(unit_price):
                        ...
                        table_item = self.table.item(row, total_col_idx)
                        if table_item is None:
                            table_item = QTableWidgetItem(str(round(total_price, 2)))
                            self.table.setItem(row, total_col_idx, table_item)
                        else:
                            table_item.setText(str(round(total_price, 2)))
