...
class StockManager(QMainWindow):
    def __init__(self, version):
        ...
        self.data_modified = False  # AÑADIR ESTA LÍNEA
    ...
    def auto_save_data(self):
        ...
        if self.data_modified:
            try:
                ...
                self.data_modified = False  # Reset flag after auto-save
    ...
    def setup_ui(self):
        ...
        # Create tabs with safe callback
    ...
    def safe_update_summary(self):
        ...
        if hasattr(self, 'summary_tab') and self.summary_tab is not None:
            ...
            print("Summary tab not ready yet")
    ...
    def safe_update_all(self):
        ...
        if hasattr(self, 'sales_tab') and self.sales_tab is not None:
            ...
            print("Tabs not ready yet")

    def update_summary_tab(self):
        """Update only the summary tab with the latest data"""
        try:
            # Reload data to ensure consistency
            self.data = self.handler.load_data()

            # Update data reference in summary tab
            if self.summary_tab:
    ...
    def update_all_data(self):
        """Update all tabs when new data is added"""
        try:
            # Reload data from file
            self.data = self.handler.load_data()

            # Update data references in all tabs
            if self.sales_tab:
                self.sales_tab.data = self.data
                self.sales_tab.load_table_data()

            if self.new_sale_tab:
                self.new_sale_tab.data = self.data
